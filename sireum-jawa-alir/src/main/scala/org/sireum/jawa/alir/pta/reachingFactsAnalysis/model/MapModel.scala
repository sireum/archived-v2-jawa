...
object MapModel {
  ...
  private def getPointStringToRet(retVar: String, currentContext: Context)(implicit factory: RFAFactFactory): RFAFact = {
    ...
    new RFAFact(VarSlot(retVar, false, false), newThisValue)   
  }
    ...
    strValue.foreach{
      s =>
        if(s.isInstanceOf[PTATupleInstance])
          result += new RFAFact(FieldSlot(rf.v, "items"), s.asInstanceOf[PTATupleInstance].left)
    }
    result
  }
      ...
      entValue.foreach{
        v =>
          ...
          if(keyValue.exists { kIns => kIns === v.asInstanceOf[PTATupleInstance].left }){
            result += (new RFAFact(VarSlot(retVar, false, false), v.asInstanceOf[PTATupleInstance].right))
          }
      }
    }
    result.toSet
