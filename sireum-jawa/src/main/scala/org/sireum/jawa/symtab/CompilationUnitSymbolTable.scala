...
import org.sireum.jawa.ClassHierarchy
import org.sireum.jawa.lexer.Token
import org.sireum.jawa.symtab.SymbolTableBuilder.CompilationUnitElementMiner
...
trait MethodSymbolTable {
  ...
  def location(locationUri: String): LocalVarDeclaration
  def catchClauses(locationIndex: Int): Iterable[CatchClause]
}

object CompilationUnitSymbolTable {
  ...
  (cus : ISeq[CompilationUnit], stpConstructor : Unit => P,
   parallel : Boolean) : CompilationUnitSymbolTableProducer = {
    ...
    ms.map { cu =>
      ...
      stp
    }.toIterable.reduce(SymbolTableBuilder.combine)
  }
      ...
      m.firstToken.fileUriOpt match {
        case Some(uri) =>
          if (changedOrDeletedCUFiles.contains(uri))
            SymbolTableBuilder.tearDown(stp.tables, m)
          else
            cus += m
        case _ =>
      }
    }
    SymbolTableBuilder.combine(stp, mineCompilationUnitElements(changedOrAddedCUs, stpConstructor, parallel))
...
(/**
  ...
  */
 catchClauses: MList[CatchClause] = mlistEmpty)