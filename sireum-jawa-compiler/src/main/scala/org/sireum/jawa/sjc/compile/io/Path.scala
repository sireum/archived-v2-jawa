...
object Path extends PathExtra
{
  ...
  private[io] def relativize(baseFile: File, file: File): Option[String] =
  {
...
object PathFinder
{
  ...
  val empty = new PathFinder { private[io] def addTo(fileSet: mutable.Set[File]) {} }
  def strict(files: Traversable[File]): PathFinder = apply(files)
  def apply(files: => Traversable[File]): PathFinder = new PathFinder {
    private[io] def addTo(fileSet: mutable.Set[File]) = fileSet ++= files
  }
...
sealed abstract class PathFinder
{
  ...
  * time of calling.  If the filesystem changes, two calls to this method might be different.*/
  ...
  private[io] def addTo(fileSet: mutable.Set[File])
...
{
  private[io] def addTo(fileSet: mutable.Set[File]): Unit = if(asFile.exists) fileSet += asFile
}
...
{
  private[io] def addTo(fileSet: mutable.Set[File]): Unit =
    for(file <- parent.get)
      handleFile(file, fileSet)
}
...
{
  private[io] def addTo(pathSet: mutable.Set[File])
  {
