...
import java.io.{BufferedReader, ByteArrayOutputStream, BufferedWriter, File => JavaFile, FileInputStream, InputStream, OutputStream, PrintWriter}
...
object IO
{
  ...
  * The name of the file will begin with `prefix`, which must be at least three characters long, and end with `postfix`, which has no minimum length.  */
  ...
  {
    val isEmpty = new HashMap[JavaFile, Boolean]
