...
object DirectoryFilter extends FileFilter {
  def accept(file: JavaFile) = file.isDirectory
}
...
{
  def accept(file: JavaFile) = acceptFunction(file)
}
