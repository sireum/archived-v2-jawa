    ...
    private def classOrInterfaceSymbol(classOrInterfaceDeclaration: ClassOrInterfaceDeclaration) = {
      ...
      ct.get(nameID.text) match {
        ...
        case _ =>
          ...
          tables.classOrInterfaceAbsTable(nameID.text.intern()) = ClassOrInterfaceSymbolTableData()
      }
    }
    ...
    private def fieldSymbol(classOrInterfaceNameID: Token, fieldDeclaration: Field with Declaration) = {
     ...
     private def methodSymbol(classOrInterfaceNameID: Token, methodDeclaration: MethodDeclaration) = {
       ...
       mt.get(methodSig) match {
         ...
         case None =>
           mt(methodSig.intern()) = methodDeclaration
           ct.methodAbsTable(methodSig.intern()) = MethodSymbolTableData()
       }
     }
     ...
     private def locationSymbol(bodyTable: MethodBodySymbolTableData, location: JawaLocation) = {
       ...
       lt.get(locationUri) match {
         case Some(other) =>
           reportRedeclaration(location.locationID.fileUriOpt, location.locationID, DUPLICATE_LOCATION, other)
         case _ =>
     ...
     private def catchSymbol(bodyTable: MethodBodySymbolTableData, catchClause: CatchClause, locations: ISeq[JawaLocation]) = {
       ...
       if (end == -1) {
         reportError(catchClause.firstToken.fileUriOpt, catchClause.range.toLocation.line,
